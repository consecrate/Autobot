...
(function () {
  function extractTex() {
    ...
    const mjx3 = document.querySelectorAll(
      "mjx-container:not([data-tex]):not([data-mathml])",
    );
    mjx3.forEach((container) => {
      if (container.closest("mjx-assistive-mml")) return;
      try {
        if (typeof MathJax !== "undefined" && MathJax?.startup?.document) {
          ...
          if (math) {
            if (math.trim().startsWith("<")) {
              // MathML input - store for downstream MathML->LaTeX conversion
